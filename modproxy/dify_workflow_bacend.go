...
func ForwardToDifyWorkFlowStream(difyURL, apiKey string, req common.ChatRequest, c *gin.Context) error {
	difyReq, err := convertToDifyWorkFlowRequest(req)
	if err != nil {
		return err
	}
...
func convertDifyWorkFlowToOllama(chunk string, model string, sb *strings.Builder, isStream bool) string {
	...
	var inputData common.DifyWorkFlowEvent
	err := json.Unmarshal([]byte(chunk), &inputData)
	if err != nil {
		return ""
	}

	workFlowData, err := json.Marshal(inputData.Data)
	...
	case "node_started":
		...
		err = parseData(inputData.Data, &nodeData)
		var preContent = "任务节点开始执行\n"
		if err == nil {
			preContent = fmt.Sprintf("任务[%d]开始,任务名:[%s]\n", nodeData.Index, nodeData.Title)
		}
		output = common.OutputData{
			Model:     model,
			CreatedAt: time.Now().Format(time.RFC3339Nano), // 当前时间
	...
	case "node_finished":
		...
		var preContent = "任务节点开始执行\n"
		if err == nil {
			preContent = fmt.Sprintf("任务[%d]结束,任务名:[%s]\n", nodeData.Index, nodeData.Title)
		}
		output = common.OutputData{
			Model:     model,
			CreatedAt: time.Now().Format(time.RFC3339Nano), // 当前时间
...
// 动态解析 data 字段
func parseData(data interface{}, out interface{}) error {
